...
// todas. NÃO faz parte do funcionamento normal do sistema, é seguro apagar esse arquivo depois
// que resolvermos o problema (26/08/2026). Não expõe token nenhum na resposta, só contagens e
// nomes.
...
export async function GET() {
  ...
  if (error || !pendente) {
    return NextResponse.json(
      { erro: "Nenhuma conexão pendente encontrada. Tenta conectar uma conta de novo primeiro (só até a tela de escolher a Página, não precisa terminar) e recarrega essa URL." },
      { status: 404 }
    );
  }
