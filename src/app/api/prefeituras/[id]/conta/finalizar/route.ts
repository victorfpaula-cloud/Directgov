...
// por Página nenhuma mensagem chega no nosso endpoint (mesma lição aprendida no Chatbot Direct).
...
// /prefeituras/[id]/conta/conectar), grava a conta de verdade em directgov_contas amarrada a essa
// prefeitura, inscreve a Página no webhook do app, e apaga a conexão pendente.
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  const formData = await request.formData();
  const idPendente = formData.get("pendente")?.toString();
  const pageIdEscolhido = formData.get("page_id")?.toString();

  const destinoBase = `/prefeituras/${params.id}/conta`;
  ...
  const { error: erroAoSalvarConta } = await admin.from("directgov_contas").upsert(
    {
      prefeitura_id: params.id,
  ...
  await admin.from("directgov_pending_connections").delete().eq("id", idPendente);
