...
export default async function ContaDaPrefeituraPage({
  params,
  searchParams,
}: {
  params: { id: string };
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-neutral-400">
        Conta conectada que recebe e responde as mensagens do Direct dessa prefeitura.
      </p>
      ...
      <div className="mt-6 flex flex-col gap-3">
        {(contas ?? []).map((conta) => (
          ...
          >
            ...
            <div className="mt-3 flex gap-2">
              <form action={`/api/prefeituras/${params.id}/conta/status`} method="POST" className="flex-1">
                <input type="hidden" name="conta_id" value={conta.id} />
                <input type="hidden" name="ativar" value={conta.active ? "0" : "1"} />
                <BotaoPausar ativo={conta.active} />
              </form>

              <form action={`/api/prefeituras/${params.id}/conta/excluir`} method="POST">
                <input type="hidden" name="conta_id" value={conta.id} />
                <button
                  type="submit"
        ...
        <a
          href={`/api/auth/facebook/start?prefeitura_id=${params.id}`}
